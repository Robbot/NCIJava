...
RainfallAvgMaxApp
		...
		-- uses the instantiable class RainfallAvgMax to compute the average of the elements from a 2D array
		-- display the average rainfall value
		-- uses the instantiable class RainfallAvgMax to  compute the maximum of the elements from a 2D array
...
public class RainfallAvgMaxApp{
	...
	public static void main(String[] args){
		...
		// create a 2D array with as many rows as the user provided for weeks and as many columns as the user provided for days
		ra = new double[weeks][days];
		...
		for (int row = 0; row < ra.length; row++){
			for (int col = 0; col < ra[row].length; col++) {
				System.out.println("enter rainfall value for week : " + (row + 1) + " day " + (1 + col));
		...
		// declare the variable myRain of RainfallAvgMin type, and create and object of tye RainfallAvgMin
		RainfallAvgMax myRain = new RainfallAvgMax();
