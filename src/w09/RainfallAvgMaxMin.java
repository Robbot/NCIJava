...
computes the minimum of the elements from a 2D array
...
public class RainfallAvgMaxMin{
	...
	// instance variable to store the min
	private double min; 
	
	// declare constructors
	public RainfallAvgMaxMin(){
	     // the instance variables are initialized with their default values, namely rain with null, and average, max and min with 0.0					
	}
	...
	// getter method to retrieve the min (i.e. the minimum)
	...
	public void calculateMax(){
		...
		for (int row = 0; row < rain.length; row++){
			for (int col = 0; col < rain[row].length; col++){
				...
				if (max < currentElement){ // if the value of max instance variable is lower than the current element from the array rain then update the max with that value -- we have just found another new maximum value
	...
	public void calculateMin(){
		// initialize the minimum with the first value of the 2D array located at row 0 and column 0
		min = rain[0][0]; 

		// to compute the minimum value of the 2D array's values traverse the 2D array, and at each iteration check how does the current value of the 2D array compare to the current minimum value stored in the min instance variable		
		for (int row = 0; row < rain.length; row++){
			for (int col = 0; col < rain[row].length; col++){
				
				
				if (min > rain[row][col]){ // if the value of min instance variable is higher than the current element from the array then update the min with that value-- we have just found another new minimum value
					min = rain[row][col]; // store the value of the current element in the min 
				}
			}
		}
	}
